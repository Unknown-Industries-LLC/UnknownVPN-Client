...
namespace Eternal_Romance
{
    #region Toggle Button

    [DefaultEvent("ToggledChanged")]
    class UK_Toggle : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            checked
            {
                ...
                G.FillPath(Gradient, (GraphicsPath)this.Pill(0, (int)Math.Round(unchecked((double)this.Height / 2.0 - (double)this.cHandle.Height / 2.0)), this.Width - 1, this.cHandle.Height - 5, new UK_Toggle.PillStyle
                {
                    Left = true,
                    Right = true
                }));
                G.DrawPath(new Pen(Color.FromArgb(64, 64, 64)), (GraphicsPath)this.Pill(0, (int)Math.Round(unchecked((double)this.Height / 2.0 - (double)this.cHandle.Height / 2.0)), this.Width - 1, this.cHandle.Height - 5, new UK_Toggle.PillStyle
                {
                    Left = true,
                    Right = true
                }));
                ...
                switch (this.ToggleType)
                {
                    ...
                    case UK_Toggle._Type.OnOff:
                        {
                            bool toggled = this.Toggled;
                            if (toggled)
                            {
                                G.DrawString("", new Font("Segoe UI", 7f, FontStyle.Regular), Brushes.DimGray, (float)(this.Bar.X + 7), (float)this.Bar.Y, new StringFormat
                                {
                                    Alignment = StringAlignment.Center,
                                    LineAlignment = StringAlignment.Center
                                });
                            ...
                            {
                                G.DrawString("", new Font("Segoe UI", 7f, FontStyle.Regular), Brushes.DimGray, (float)(this.Bar.X + 18), (float)this.Bar.Y, new StringFormat
                                {
                                    Alignment = StringAlignment.Center,
                                    LineAlignment = StringAlignment.Center
                                });
